...
namespace AMT.Common
{
    public static class ConnectionManagerUtil
    {
      ...
      public static List<ProfileConnectionInfo > GetProfileConnectionInfo ()
      {
          try
          {
              for (int i = 0; i < ProfileConnectionInfo.Count; i++)
              {
                  ProfileConnectionInfo[i].Hash = Utility.HashPassword(ProfileConnectionInfo[i].Password);
                  ProfileConnectionInfo[i].Password = Utility.Encrypt(ProfileConnectionInfo[i].Password, ProfileConnectionInfo[i].Hash);
      ...
      public static void   SetProfileConnectionInfo (List<ProfileConnectionInfo > items)
      {
          try
          {
              for (int i = 0; i < items.Count; i++)
              {
                  ...
                  if (!String.IsNullOrEmpty(items[i].Password))
                      items[i].Password = Utility.Decrypt(items[i].Password, items[i].Hash);
